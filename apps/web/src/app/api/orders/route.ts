...
// ─── Local types for the variant query result ────────────────────────────────
...
export async function POST(request: Request) {
  ...
  if (variantsError || !rawVariants?.length) {
    return NextResponse.json({ error: 'Товары не найдены' }, { status: 400 });
  }

  // Cast to explicit local type — Supabase's deep join inference collapses to never
  const variants = rawVariants as unknown as VariantRow[];

  // Build items with DB prices
  const itemsWithDetails = cartItems
    .map((cartItem) => {
      const v = variants.find((vv) => vv.id === cartItem.variantId);
      if (!v) return null;
      const effectivePrice = v.sale_price_rub ?? v.price_rub;
      return { cartItem, variant: v, effectivePrice };
    })
    .filter((x): x is { cartItem: CartItemInput; variant: VariantRow; effectivePrice: number } => x !== null);
  ...
  const orderItems: OrderItemInsert[] = itemsWithDetails.map(
    ({ cartItem, variant, effectivePrice }) => {
  ...
  if (itemsError) {
    ...
    // Order created — return orderId even if items insert failed
  }
