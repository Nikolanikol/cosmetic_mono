...
const RAW_DIR = path.join(__dirname, 'torriden-raw');
...
function guessCategoryByTitle(title) {
  ...
  if (t.includes('cleanser') || t.includes('cleansing') || t.includes('foam') || t.includes('wash')) return 'cleanser';
...
async function main() {
  ...
  if (files.length === 0) {
    console.error('No product JSON files found in torriden-raw/. Run scrape-torriden.mjs first.');
    process.exit(1);
  }
  ...
  const brandId = brandMap['torriden'];
  if (!brandId) {
    console.error('Brand "torriden" not found in brands table.');
    process.exit(1);
  }
  ...
  for (const file of files) {
    ...
    const sourceUrl = `https://torriden.us/products/${handle}`;
